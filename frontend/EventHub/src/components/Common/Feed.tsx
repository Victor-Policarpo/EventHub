...
export function Feed({ title, filterBar, children, pagination, isEmpty, emptyMessage }: FeedProps) {
    return (
        <div className="w-full max-w-5xl mx-auto px-4 py-6 md:px-8 md:py-8 flex flex-col gap-6">
            ...
            <div className={`flex flex-col gap-4 transition-opacity duration-200 ${pagination?.isPlaceholderData ? 'opacity-50' : 'opacity-100'}`}>
                {isEmpty ? (
                ...
                <div className="flex items-center justify-between border-t border-slate-200 pt-6 mt-2">
                    <div className="flex flex-col gap-0.5">
                        <span className="text-sm text-slate-700">
                            Página <strong className="font-semibold">{pagination.currentPage + 1}</strong> de <strong className="font-semibold">{pagination.totalPages}</strong> 
                        </span>
                    ...
                    <div className="flex gap-2">
                        <Button
                            variant="ghost"
                            disabled={pagination.currentPage === 0 || pagination.isPlaceholderData}
                        ...
                        <Button
                            variant="secondary"
                            disabled={pagination.currentPage >= pagination.totalPages - 1 || pagination.isPlaceholderData}
                            onClick={() => pagination.onPageChange(pagination.currentPage + 1)}
                            className="w-10 h-10 p-0 flex items-center justify-center rounded-lg"
